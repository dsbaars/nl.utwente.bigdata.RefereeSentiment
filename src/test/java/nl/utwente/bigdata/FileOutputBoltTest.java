...
public class FileOutputBoltTest {
	private FileOutputBolt bolt; 
	private BasicOutputCollector collector;
	private OutputCollector output;
	private Collector col;
	private Config config = new Config();
	private TopologyContext context;
	private static Logger log = LoggerFactory.getLogger(FileOutputBoltTest.class);
	...
	private Tuple generateTestTuple(Object tweet) {
        TopologyBuilder builder = new TopologyBuilder();
        GeneralTopologyContext topologyContext = new GeneralTopologyContext(builder.createTopology(), new Config(), new HashMap(), new HashMap(), new HashMap(), "") {
            @Override
            public Fields getComponentOutputFields(String componentId, String streamId) {
                return new Fields("tweet", "normalized_text", "sentiment", "home", "away");
            }
        };
        return new TupleImpl(topologyContext, new Values(tweet, ((Status)tweet).getText(), 0, "NED", "SPA"), 1, "");
    }
