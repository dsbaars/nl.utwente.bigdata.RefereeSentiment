package nl.utwente.bigdata.utils;
...
public class Emoji {
	...
	private static class FlagReplacementMap extends HashMap<String,List<Integer>> {
		
		private static final long serialVersionUID = -2236378897447743905L;
		private static FlagReplacementMap mInstance;

		private FlagReplacementMap()
		{
			put("de", Arrays.asList(0x1F1E9, 0x1F1EA));
			put("nl", Arrays.asList(0x1F1E9, 0x1F1EA));
			put("es", Arrays.asList(0x1F1EA, 0x1F1F8));
			put("fr", Arrays.asList(0x1F1EB, 0x1F1F7));
			put("en", Arrays.asList(0x1F1EC, 0x1F1E7));
			put("nl", Arrays.asList(0x1F1F3, 0x1F1F1));
		}
		
		public static FlagReplacementMap getInstance() {
			if (mInstance == null) {
				mInstance = new FlagReplacementMap();
			}
			return mInstance;
		}
	}
	...
	public static String replaceFlagInText(String text) {
		final FlagReplacementMap flagReplacements = FlagReplacementMap.getInstance();
		String emoticon;
		List<Integer> codepoint;

		for (Map.Entry<String, List<Integer>> entry : flagReplacements.entrySet()) {
			if (entry != null) {
				emoticon = entry.getKey();
				codepoint = entry.getValue();
				if (emoticon != null && codepoint != null) {
					String unicodeChar = getUnicodeChar(codepoint.get(0).intValue()) + getUnicodeChar(codepoint.get(1).intValue());
					text = text.replaceAll(getEmoticonSearchRegex(emoticon), unicodeChar);
				}
			}
		}
